...
const IPAChart = () => {
  ...
  //   setClicks({ ...clicks, [symbol]: clicks[symbol] + 1 });
  //   setCurrentSymbol(symbol);
  //   setCurrentContent(getContent(symbol, clicks[symbol]));
  // };
  const handleClick = (symbol) => {
  ...
  const [consecutiveDCount, setConsecutiveDCount] = useState(0);

const handleKeyDown = (event) => {
  const key = event.key.toLowerCase();
  if (key === 'd') {
    setConsecutiveDCount((prevCount) => prevCount + 1);
    const clickCount = consecutiveDCount % 4;
    const symbol = 'd'; // Your specific symbol here
    handleClick(symbol, clickCount);
  } else {
    setConsecutiveDCount(0);
  }
};
  ...
  const getContent = (symbol, clickCount) => {
    switch (clickCount) {
      case 0:
        return `Sound: ${symbol}`;
      case 1:
        return `Examples: ${examplesMonophthongs[symbol] || examplesConsonants[symbol]}`;
      case 2:
        return `Description: ${descriptionsMonophthongs[symbol] || descriptionsConsonants[symbol]}`;
      case 3:
        return `Braille Representation: ${brailleRepresentationsMonophthongs[symbol] || brailleRepresentationsConsonants[symbol]}`;
      default:
        return '';
    }
  };
  ...
  useEffect(() => {
    const handleKeyPress = (event) => {
      ...
      if (allSymbols.includes(key)) {
        ...
        const contentToSpeak = `Example: ${examplesMonophthongs[key] || examplesConsonants[key]}. 
        Description: ${descriptionsMonophthongs[key] || descriptionsConsonants[key]}. Braille Representation: ${brailleRepresentationsMonophthongs[key] || brailleRepresentationsConsonants[key]}`;
        speakText(contentToSpeak);
      }
    };
...
//   const handleClick = (symbol) => {
//     const clickCount = clicks[symbol] + 1;
//     setClicks({ ...clicks, [symbol]: clickCount });
//     setCurrentSymbol(symbol);
//     setCurrentContent(getContent(symbol, clickCount));

//     const contentToSpeak = `Example: ${examplesMonophthongs[symbol] || examplesConsonants[symbol]}`;
//     speakText(contentToSpeak);
// };
  ...
  return (
    <div className="ipa-chart">
      ...
      <h2>Monophthongs</h2>
      <div className="monophthongs-section">
        {monophthongs.map((symbol) => (
          <button
            key={symbol}
            className="monophthong-button"
      ...
      <div className="content-section">
        {currentContent && (
          <>
            ...
            <div>
              <h3>Examples:</h3>
              <p>{examplesMonophthongs[currentSymbol] || examplesConsonants[currentSymbol]}</p>
              <h3>Description:</h3>
              <p>{descriptionsMonophthongs[currentSymbol] || descriptionsConsonants[currentSymbol]}</p>
              <h3>Braille Representation:</h3>
              <p>{brailleRepresentationsMonophthongs[currentSymbol] || brailleRepresentationsConsonants[currentSymbol]}</p>
            </div>
          </>
        )}
      </div>
